...
class QuizManager:
    ...
    def create_quiz(
        ...
        with Database() as db:
            ...
            """,
                (session_id, course_code, quiz_type, topic_id, knowledge_item_id, len(exercises)),
            )
    ...
    def _select_exercises(
        ...
        if quiz_type == "topic" and topic_id:
            ...
            conditions.append("(sp.next_review IS NULL OR sp.next_review <= datetime('now'))")
    ...
    def submit_answer(
        ...
        with Database() as db:
            ...
            quality = self.sm2.convert_score_to_quality(score=score, hint_used=hint_used)
            ...
            if knowledge_item_id:
                sm2_update = self._update_sm2_progress(
                    db, knowledge_item_id, exercise["course_code"], quality, is_correct
                )
            ...
            """,
                (user_answer, is_correct, score, hint_used, time_taken, session_id, exercise_id),
            )
    ...
    def complete_quiz(self, session_id: str) -> Dict[str, Any]:
        ...
        with Database() as db:
            ...
            if not stats or stats["total_questions"] == 0:
                return {"completed": False, "error": "No answers found for this quiz"}
